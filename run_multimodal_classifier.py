...
class MMInputExample(object):
    """A single training(with image)/test example for simple sequence classification."""
...
def main():
    ...
    if args.mm_model == 'ResBert':
        ...
        model = TomBertForMMSequenceClassification.from_pretrained('bert-base-cased',#args.bert_model,
                                                                cache_dir=PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(
                                                                    args.local_rank),
                                                                num_labels=num_labels,
                                                                pooling=args.pooling)
